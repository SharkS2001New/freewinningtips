...
const specificJackpotTips = () => {
  const [jackpotPick17Tips, setJackpotPick17Tips] = useState([]);
  const [jackpotPick13Tips, setJackpotPick13Tips] = useState([]);
  ...
  useEffect(() => {
    fetchPick17JackpotFixtures('Betpawa Pick 17 Jackpot');
    fetchPick13JackpotFixtures('Betpawa Pick13 Jackpot');
  }, [selectedJackpot]);

  const headers = { "Partner-Authorization": "q2LsJ9FmT6XvRaCbHuYdK8ZwN4" };

  const fetchPick17JackpotFixtures = async (jackpotName) => {
    try {
      ...
      if (data.status && data.data) {
        setJackpotPick17Tips(data.data);
      }
    } catch (error) {
  ...
  const fetchPick13JackpotFixtures = async (jackpotName1) => {
    try {
      const response = await fetch(`https://api.pitchpredictions.com/api/fetch_jackpot_fixtures_by_name?jackpot_name=${jackpotName1}`, {
        headers: headers
      });
      const data = await response.json();

      if (data.status && data.data) {
        setJackpotPick13Tips(data.data);
      }
    } catch (error) {
      console.error('Error fetching jackpot fixtures:', error);
    }
  ...
  return (
    <React.Fragment>
      <div className="jackpot-tips-section">
        {jackpotPick17Tips.length > 0 || jackpotPick13Tips.length > 0 ? (
          <div ref={jackpotTableRef} className="jackpot-table mb-5">
            ...
            {/* Add buttons to scroll to Pick17 and Pick13 */}
            <br />
            <div className="row">
              <div className="col-md-3 col-1"></div>
              <div className="col">
                <button className="btn btn-success" onClick={() => window.location.href = '#Pick17'}>Scroll to Pick 17</button>
              </div>
              <div className="col">
                <button className="btn btn-primary" onClick={() => window.location.href = '#Pick13'}>Scroll to Pick 13</button>
              </div>
              <div className="col-md-3 col-1"></div>
            </div>
            <br />
            {/* Add an anchor tag for Pick17 access */}
            <a id="Pick17"></a>
            <h2 className="sectionTitle text-center">Free Betpawa Pick 17 Jackpot Predictions</h2>
            {jackpotPick13Tips.length > 0 ? (
              <h3 className="sectionTitle text-center">(Starts At: {DateTimeToUsersTimezone(getMinMaxDates(jackpotPick17Tips)["minDate"])} - Ends At: {DateTimeToUsersTimezone(getMinMaxDates(jackpotPick17Tips)["maxDate"])})</h3>
            ) : (
              <></>
            )}
            <table className="styled-table">
              ...
              <tbody>
                {jackpotPick17Tips.map((jackpot, index) => (
            ...
            <br/>
            <br/>
            {/* Add an anchor tag for Pick13 access */}
            <a id="Pick13"></a>
            <h2 className="sectionTitle text-center">Free Betpawa Pick 13 Jackpot Predictions</h2>
            {jackpotPick13Tips.length > 0 ? (
              <h3 className="sectionTitle text-center">(Starts At: {DateTimeToUsersTimezone(getMinMaxDates(jackpotPick13Tips)["minDate"])} - Ends At: {DateTimeToUsersTimezone(getMinMaxDates(jackpotPick13Tips)["maxDate"])})</h3>
            ) : (
              <></>
            )}
            <table className="styled-table">
              ...
              <tbody>
                {jackpotPick13Tips.map((jackpot1, index1) => (
                  (winningtip1 = WinningTeamPred1x2(jackpot1.percent_pred_home, jackpot1.percent_pred_draw, jackpot1.percent_pred_away, jackpot1.goals_home, jackpot1.goals_away)),
                  <tr key={jackpot1.id}>
                    <td>{index1 + 1}</td>
                    <td>{jackpot1.home_team_name} <span style={{ color: 'red' }}>VS</span> &nbsp; {jackpot1.away_team_name}</td>
                    <td style={{ textAlign: "center" }}>
                      <div style={{ display: 'inline-block', marginRight: '4px', fontWeight: "bold", fontSize: "15px" }}>{winningtip1[0]}</div>&nbsp;
                      <div style={{ display: 'inline-block' }}>{winningtip1[1]}</div>
                    </td>
                    <td>{DateTimeToUsersTimezone(jackpot1.date)}</td>
                    <td className="text-center">
                      {jackpot1.goals_home !== null && jackpot1.goals_away !== null
                        ? `${jackpot1.goals_home} - ${jackpot1.goals_away}`
                        : "-"}
                    </td>
                  </tr>
                ))}
