...
import MwanasokaPageContent from '@/components/seo-content/mwanasoka-content';

// Shared cache config — must match other pages exactly
const CACHE_DIR    = path.join(process.cwd(), 'public', 'cache', 'pages-data');
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

export default function MwanasokaPage({ fixtures, fetchDate }) {
  return (
...
export async function getServerSideProps() {
  ...
  try {
    ...
    console.error('[mwanasoka-page] getServerSideProps error:', err.message);
