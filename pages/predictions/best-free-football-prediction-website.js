...
import BestFootballPredictionsPageContent from '@/components/seo-content/best-football-prediction-content';

// Shared cache config — must match other pages exactly
const CACHE_DIR    = path.join(process.cwd(), 'public', 'cache', 'pages-data');
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

export default function BestFootballPredictions({ fixtures, fetchDate }) {
  return (
    <div className="page-root">
        ...
        {/* SEO CONTENT */}
        <BestFootballPredictionsPageContent/>
      </div>
  )
}
...
export async function getServerSideProps() {
  ...
  try {
    ...
    console.error('[best-football-predictions] getServerSideProps error:', err.message);
