...
import FreeFootballPredictionsTodayPageContent from '@/components/seo-content/free-football-prediction-today-content';

// Shared cache config — must match other pages exactly
const CACHE_DIR    = path.join(process.cwd(), 'public', 'cache', 'pages-data');
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

export default function FreeFootballPredictionsTodayPage({ fixtures, fetchDate }) {
  return (
    <div className="page-root">
        ...
        {/* SEO CONTENT */}
        <FreeFootballPredictionsTodayPageContent/>
      </div>
  )
}
...
export async function getServerSideProps() {
  ...
  try {
    ...
    console.error('[free-football-predictions-today] getServerSideProps error:', err.message);
