...
import SokafansPageContent from '@/components/seo-content/sokafans-content';

// Shared cache config — must match other pages exactly
const CACHE_DIR    = path.join(process.cwd(), 'public', 'cache', 'pages-data');
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

export default function SokafansPage({ fixtures, fetchDate }) {
  return (
...
export async function getServerSideProps() {
  ...
  try {
    ...
    console.error('[sokafans-page] getServerSideProps error:', err.message);
